...
A portable AI skill for deep work over heavy inputs — PDF analysis, large dataset/log analysis, web scraping, and **web search** — without losing accuracy.
...
│   ├── search.md               # web search (Tavily/Exa/Brave/SerpAPI): fetch wide, inject narrow
...
│   └── search.py               # token-saving web search: passage slice + dedup + budget + reversible cache
├── eval/
...
# Web search: fetch wide, inject narrow (compact, cited slice)
export TAVILY_API_KEY=tvly-...
python scripts/search.py query "GDPR data residency requirements 2026" \\
  --provider tavily --max-results 8 --budget 1200 -o hits.md
# inside an agent with a search connector (e.g. Tavily MCP), pipe its JSON instead of a key:
echo "$CONNECTOR_JSON" | python scripts/search.py query "..." --input - --budget 1200
```

`retrieve.py` uses semantic matching automatically if `sentence-transformers` is installed; otherwise it falls back to keyword + structural anchors. `search.py` reuses the same scorer.
...
The same pattern applies to a million-row CSV (a checkable aggregate, not the rows), a 100-page codebase (the relevant files, not the tree), and web search (the answering passages, not every result body).
...
The web-search path (`search.py`) was separately verified end-to-end, including the connector-bridge (`--input -`) path that runs inside Claude/Cowork without a shell API key: 90–98% smaller than dumping every result. Reproducible from a clean checkout in a few minutes (no API key needed for the token measurement). Full methodology, per-check breakdown, and a known limitation are in TESTING.md.
